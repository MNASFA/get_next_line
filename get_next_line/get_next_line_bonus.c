...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/28 12:53:30 by hmnasfa           #+#    #+#             */
/*   Updated: 2024/11/28 14:07:30 by hmnasfa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!tmp_buffer)
		return (free(banks[fd]), NULL);
	res = handle_read(fd, &banks[fd], tmp_buffer);
	free(tmp_buffer);
	if (res == -1 || (res == 0 && (!banks[fd] || !*banks[fd])))
		return (free(banks[fd]), banks[fd] = NULL, NULL);
	return (extract_line(&banks[fd]));
}
